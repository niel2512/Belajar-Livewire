<div class="w-1/2 m-auto my-10">
    ...
    <div class="py-2">
        ...
        {{-- Flash Message --}}
        @if (session('sukses'))
            <div class="p-4 mt-5 mb-4 text-sm text-green-800 rounded-lg bg-green-100" role="alert">
                {{ session('sukses') }}
            </div>
        @endif
        ...
        <div class="mt-4">
            <form wire:submit="createUser" action="#" method="POST" class="space-y-6">
                ...
                <div class="col-span-full">
                    <label for="foto-profil" class="block text-sm/6 font-medium text-gray-900">Foto Profil</label>
                ...
                {{-- Nambahin loading preview --}}
                <div wire:loading wire:target="avatar"
                    class="flex items-center justify-center bg-neutral-secondary-soft h-20 w-20 border rounded-full text-fg-brand-strong text-xs border-gray-200 bg-gray-50">
                    <div class="px-3 py-7 font-medium animate-pulse">
                        loading...</div>
                </div>

                {{-- Preview gambar --}}
                @if ($avatar)
                    <p class="my-2 text-sm/6 font-medium">Preview Foto</p>
                    <img src="{{ $avatar->temporaryUrl() }}" class="rounded-full w-20 h-20 block object-cover">
                @endif
                <div>
                    <button wire:click.prevent="createUser"
                        ...
                        <span class="sr-only">Loading...</span>Create
                        User</button>
                </div>
            </form>
        </div>
